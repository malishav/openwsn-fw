...
// maximum cell list length
#define CELL_LIST_MAX_LEN 5
...
// component identifiers, the order is important
enum {
    COMPONENT_NULL                      = 0x00,
...
enum {
    ...
    ERR_UNEXPECTED_DAO                  = 0x0e, // unexpected DAO (code location {0}). A change maybe happened on
                                                // dagroot node.
    ...
    ERR_LOOP_DETECTED                   = 0x15, // loop detected due to previous rank {0} lower than current node rank
                                                // {1}
