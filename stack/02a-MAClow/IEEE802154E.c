...
bool isValidAck(ieee802154_header_iht *ieee802514_header, QueueEntry_t *packetSent);

bool isValidJoin(QueueEntry_t *eb, ieee802154_header_iht *parsedHeader);

bool isValidEbFormat(QueueEntry_t *pkt, uint16_t *lenIE);

// IEs Handling
bool ieee154e_processIEs(QueueEntry_t *pkt, uint16_t *lenIE);
...
void notif_sendDone(QueueEntry_t *packetSent, owerror_t error);
...
port_INLINE void activity_synchronize_endOfFrame(PORT_TIMER_WIDTH capturedTime) {
    ...
    ieee154e_vars.dataReceived = queue_getFreePacketBuffer(COMPONENT_IEEE802154E);
    ...
    */
    ...
    queue_freePacketBuffer(ieee154e_vars.dataReceived);
...
port_INLINE bool ieee154e_processIEs(QueueEntry_t* pkt, uint16_t* lenIE) {
    uint8_t i;
        ...
        case CELLTYPE_TX:
            ...
            if (schedule_getOkToSend()) {
                if (packetfunctions_isBroadcastMulticast(&neighbor) == FALSE) {
                    ...
                    ieee154e_vars.dataToSend = queue_macGetUnicastPacket(&neighbor);

                    if (ieee154e_vars.dataToSend == NULL) {
                        ieee154e_vars.dataToSend = queue_macGetKaPacket(&neighbor);
                    }
                    ...
                    ieee154e_vars.dataToSend = queue_macGetDIOPacket();
                    if (ieee154e_vars.dataToSend == NULL) {
                        couldSendEB = TRUE;
                        // look for an EB packet in the queue
                        ieee154e_vars.dataToSend = queue_macGetEBPacket();
                    }
                }
            }
...
port_INLINE void activity_ti9(PORT_TIMER_WIDTH capturedTime) {
    ...
    ieee154e_vars.ackReceived = queue_getFreePacketBuffer(COMPONENT_IEEE802154E);
    ...
    */
    ...
    queue_freePacketBuffer(ieee154e_vars.ackReceived);
...
port_INLINE void activity_ri6(void) {
    ...
    // get a buffer to put the ack to send in
    ieee154e_vars.ackToSend = queue_getFreePacketBuffer(COMPONENT_IEEE802154E);
    if (ieee154e_vars.ackToSend == NULL) {
        // log the error
        LOG_ERROR(COMPONENT_IEEE802154E, ERR_NO_FREE_PACKET_BUFFER, (errorparameter_t)0, (errorparameter_t)0);
        // indicate we received a packet anyway (we don't want to loose any)
    ...
    if (ieee154e_vars.ackToSend->l2_securityLevel != IEEE154_ASH_SLF_TYPE_NOSEC) {
        if (IEEE802154_security_outgoingFrameSecurity(ieee154e_vars.ackToSend) != E_SUCCESS) {
            queue_freePacketBuffer(ieee154e_vars.ackToSend);
            endSlot();
            return;
        }
    }
...
*/
port_INLINE bool isValidAck(ieee802154_header_iht* ieee802514_header, QueueEntry_t* packetSent) {
...
void endSlot(void) {
    ...
    if (
        ...
        if (queue_macGetUnicastPacket(&slotNeighbor) == NULL) {
