...
void msf_trigger6pAdd(void) {
    open_addr_t neighbor;
    bool foundNeighbor;
    cellInfo_ht celllist_add[CELL_LIST_MAX_LEN];
...
void msf_trigger6pDelete(void) {
    open_addr_t neighbor;
    bool foundNeighbor;
    cellInfo_ht celllist_delete[CELL_LIST_MAX_LEN];
...
bool msf_candidateAddCellList(
    ...
    memset(cellList, 0, CELL_LIST_MAX_LEN * sizeof(cellInfo_ht));
    numCandCells = 0;
...
bool msf_candidateRemoveCellList(
    ...
    for (i = 0; i < schedule_getFrameLength(); i++) {
        ...
        if (
            ...
            cellList[numCandCells].isUsed = TRUE;
            numCandCells++;
            if (numCandCells == CELL_LIST_MAX_LEN) {
                break;
            }
        }
    }
...
void msf_housekeeping(void) {
    ...
    memset(celllist_delete, 0, CELL_LIST_MAX_LEN * sizeof(cellInfo_ht));
