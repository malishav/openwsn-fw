...
typedef struct {
    ...
    uint8_t     numTx;
    uint8_t     numTxACK;
    uint8_t     numWraps;    // number of times the tx counter wraps. Remove to save memory.
    asn_t       asn;
    uint8_t     joinPrio;
    bool        f6PNORES;
...
void neighbors_indicateRx(open_addr_t* l2_src, int8_t rssi, asn_t* asnTimestamp, bool joinPrioPresent,
                          uint8_t joinPrio, bool insecure);
void neighbors_indicateTx(open_addr_t* dest, uint8_t numTxAttempts, bool sentOnTxCell, bool was_finally_acked,
                          asn_t* asnTimestamp);
