...
// send internal
owerror_t sixtop_send_internal(QueueEntry_t *msg, bool payloadIEPresent);
...
void sixtop_six2six_sendDone(QueueEntry_t *msg, owerror_t error);
...
owerror_t sixtop_request(
    ...
    if (queue_getNum6PReq(neighbor) > 0) {
        // remove previous request as it's not sent out
        queue_remove6PrequestToNeighbor(neighbor);
    }

    // get a free packet buffer
    pkt = queue_getFreePacketBuffer(COMPONENT_SIXTOP_RES);
    if (pkt == NULL) {
    ...
    if (celllist_toBeDeleted != NULL) {
        memcpy(sixtop_vars.celllist_toDelete, celllist_toBeDeleted, CELL_LIST_MAX_LEN * sizeof(cellInfo_ht));
    }
...
owerror_t sixtop_send(QueueEntry_t *msg) {
...
void task_sixtopNotifReceive(void) {
    ...
    msg = queue_sixtopGetReceivedPacket();
    if (msg == NULL) {
...
port_INLINE void sixtop_sendEB(void) {
    ...
    if (
        ...
        queue_removeAllCreatedBy(COMPONENT_SIXTOP);
    ...
    eb = queue_getFreePacketBuffer(COMPONENT_SIXTOP);
    if (eb == NULL) {
...
port_INLINE void sixtop_sendKA(void) {
    ...
    // get a free packet buffer
    kaPkt = queue_getFreePacketBuffer(COMPONENT_SIXTOP);
    if (kaPkt == NULL) {
...
port_INLINE bool sixtop_processIEs(QueueEntry_t* pkt, uint16_t* lenIE) {
    uint8_t ptr;
    uint8_t temp_8b;
        ...
        if (code == IANA_6TOP_RC_SUCCESS || code == IANA_6TOP_RC_EOL) {
            switch (sixtop_vars.six2six_state) {
                ...
                case SIX_STATE_WAIT_LISTRESPONSE:
                    i = 0;
                    memset(celllist_list, 0, CELL_LIST_MAX_LEN * sizeof(cellInfo_ht));
                    while (pktLen > 0) {
                ...
                default:
                    ...
                    queue_remove6PrequestToNeighbor(&(pkt->l2_nextORpreviousHop));
                    break;
            }
        } else {
...
bool sixtop_areAvailableCellsToBeScheduled(
    ...
    if (numOfCells == 0 || numOfCells > CELL_LIST_MAX_LEN) {
...
bool sixtop_areAvailableCellsToBeRemoved(
    ...
    if (numOfCells == 0 || numOfCells > CELL_LIST_MAX_LEN) {
        ...
        do {
            ...
            i++;
        } while (i < CELL_LIST_MAX_LEN && numOfavailableCells < numOfCells);
