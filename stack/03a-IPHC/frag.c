...
static void store_fragment(QueueEntry_t *msg, uint16_t size, uint16_t tag, uint8_t offset);

static void reassemble_fragments(uint16_t tag, uint16_t size, QueueEntry_t *reassembled_msg);

static owerror_t allocate_vrb(QueueEntry_t *frag1, uint16_t size, uint16_t tag);

static void prepend_frag1_header(QueueEntry_t *frag1, uint16_t size, uint16_t tag);

static void prepend_fragn_header(QueueEntry_t *fragn, uint16_t size, uint16_t tag, uint8_t offset);
...
void frag_sendDone(QueueEntry_t *msg, owerror_t sendError) {
    uint32_t i, k;
    bool frags_queued;
    bool upward_relay;
    uint16_t datagram_tag;
    QueueEntry_t *original_msg;
