...
owerror_t iphc_prependIPv6HopByHopHeader(QueueEntry_t **msg, uint8_t nextheader, rpl_option_ht *rpl_option);

#if DEADLINE_OPTION
// IPv6 Deadline hop-by-hop header
owerror_t iphc_prependIPv6DeadlineHeader(QueueEntry_t **msg);
...
owerror_t iphc_sendFromBridge(QueueEntry_t *msg) {
    msg->owner = COMPONENT_IPHC;
    // error checking
...
void iphc_sendDone(QueueEntry_t *msg, owerror_t error) {
    msg->owner = COMPONENT_IPHC;
...
void iphc_receive(QueueEntry_t *msg) {
    ipv6_header_iht ipv6_outer_header;
    ipv6_header_iht ipv6_inner_header;
    uint8_t page_length;
    rpl_option_ht rpl_option;
...
owerror_t iphc_retrieveIPv6Header(QueueEntry_t *msg, ipv6_header_iht *ipv6_outer_header,
...
*/
uint8_t iphc_retrieveIPv6HopByHopHeader(QueueEntry_t *msg, rpl_option_ht *rpl_option) {
    uint8_t temp_8b;
    uint8_t type;
    uint8_t length;
