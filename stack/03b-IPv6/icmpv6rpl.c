...
*/
void icmpv6rpl_sendDone(QueueEntry_t *msg, owerror_t error) {
...
*/
void icmpv6rpl_receive(QueueEntry_t *msg) {
    uint8_t icmpv6code;
...
void icmpv6rpl_updateNexthopAddress(open_addr_t *newParent) {
    ...
    queue_updateNextHopPayload(newParent);
}
...
\param[in] msg The received message with msg->payload pointing to the DIO
   header.
*/
void icmpv6rpl_indicateRxDIO(QueueEntry_t *msg) {
    uint8_t i;
    uint8_t temp_8b;
    dagrank_t neighborRank;
...
void sendDIO(void) {
    ...
    if (ieee154e_isSynch() == FALSE) {
        ...
        queue_removeAllCreatedBy(COMPONENT_ICMPv6RPL);
    ...
    msg = queue_getFreePacketBuffer(COMPONENT_ICMPv6RPL);
    if (msg == NULL) {
...
*/
void sendDAO(void) {
    QueueEntry_t *msg;                // pointer to DAO messages
