...
int sock_udp_send(sock_udp_t* sock, const void* data, size_t len, const sock_udp_ep_t* remote) {
    ...
    if ((pkt = queue_getFreePacketBuffer(COMPONENT_SOCK_TO_UDP)) == NULL) {
        return -ENOMEM;
    }
...
int sock_udp_recv(sock_udp_t* sock, void* data, size_t max_len, uint32_t timeout, sock_udp_ep_t* remote) {
    ...
    queue_freePacketBuffer(sock->txrx);
...
void sock_receive_internal(void) {
    ...
    pkt = queue_getPacketByComponent(COMPONENT_UDP_TO_SOCK);
...
void sock_senddone_internal(QueueEntry_t* msg, owerror_t error) {
    ...
    pkt = queue_getPacketByComponent(COMPONENT_UDP);
...
void _sock_transmit_internal(void) {
    ...
    pkt = queue_getPacketByComponent(COMPONENT_SOCK_TO_UDP);
