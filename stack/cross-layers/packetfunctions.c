...
owerror_t packetfunctions_writeAddress(QueueEntry_t **msg, open_addr_t *address, bool littleEndian) {
    uint8_t i;
    uint8_t address_length;
...
owerror_t packetfunctions_reserveHeader(QueueEntry_t **pkt, uint16_t header_length) {
    ...
    if ((*pkt)->is_big_packet == FALSE && ((*pkt)->length + header_length) > available_bytes) {
        ...
        memcpy(bpkt, (*pkt), sizeof(QueueEntry_t));
...
void packetfunctions_tossHeader(QueueEntry_t **pkt, uint16_t header_length) {
#if OPENWSN_6LO_FRAGMENTATION_C
    int16_t available_bytes;
    available_bytes = IEEE802154_FRAME_SIZE - LENGTH_CRC - IEEE802154_SECURITY_TAG_LEN;
    ...
    if ((*pkt)->is_big_packet == FALSE) {
        ...
        if ((*pkt)->length - header_length <= available_bytes){
            ...
            memcpy(spkt, (*pkt), sizeof(QueueEntry_t));
...
owerror_t packetfunctions_reserveFooter(QueueEntry_t **pkt, uint16_t footer_length) {
...
void packetfunctions_tossFooter(QueueEntry_t **pkt, uint16_t footer_length) {
    (*pkt)->length -= footer_length;
...
void packetfunctions_duplicatePacket(QueueEntry_t *dst, QueueEntry_t *src) {
    ...
    memcpy(dst, src, sizeof(QueueEntry_t));
...
void packetfunctions_calculateCRC(QueueEntry_t *msg) {
    uint16_t crc;
    uint8_t i;
    uint8_t count;
    crc = 0;
...
bool packetfunctions_checkCRC(QueueEntry_t *msg) {
    uint16_t crc;
    uint8_t i;
    uint8_t count;
    crc = 0;
...
void packetfunctions_calculateChecksum(QueueEntry_t *msg, uint8_t *checksum_ptr) {
