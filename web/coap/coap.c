...
void coap_receive(QueueEntry_t *msg);

void coap_sendDone(QueueEntry_t *msg, owerror_t error);

owerror_t coap_header_encode(QueueEntry_t *msg,
                             uint8_t version,
                             coap_type_t type,
                             uint8_t TKL,
                             coap_code_t code,
...
void coap_handle_proxy_scheme(QueueEntry_t *msg,
                              coap_header_iht *header,
                              coap_option_iht *incomingOptions,
                              uint8_t incomingOptionsLen);

void coap_handle_stateless_proxy(QueueEntry_t *msg,
                                 coap_header_iht *header,
                                 coap_option_iht *incomingOptions,
                                 uint8_t incomingOptionsLen);
...
void coap_forward_message(QueueEntry_t *msg,
                          coap_header_iht *header,
...
owerror_t coap_sock_send_internal(QueueEntry_t *msg);
...
void coap_receive(QueueEntry_t *msg) {
    ...
    if (found == TRUE && securityReturnCode == COAP_CODE_EMPTY) {
        ...
        if (temp_desc->securityContext != NULL) {
            ...
            // protect the message in the queue buffer
    ...
    // once header is reserved, encode the options to the queue payload buffer
...
*/
void coap_writeLinks(QueueEntry_t *msg, uint8_t componentID) {
    coap_resource_desc_t *temp_resource;
...
void coap_sock_handler(sock_udp_t *sock, sock_async_flags_t type, void *arg) {
    ...
    if (type & SOCK_ASYNC_MSG_RECV) {
        ...
        msg = queue_getFreePacketBuffer(COMPONENT_OPENCOAP);
        ...
        msg = queue_getPacketByComponent(COMPONENT_OPENCOAP);
...
void coap_forward_message(QueueEntry_t *msg,
    ...
    QueueEntry_t *outgoingPacket;

    outgoingPacket = queue_getFreePacketBuffer(COMPONENT_OPENCOAP);
    ...
    fail:
    queue_freePacketBuffer(outgoingPacket);
}
