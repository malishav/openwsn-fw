...
#define EAAD_MAX_LEN            (9 + OSCORE_MAX_ID_LEN)     // assumes no Class I options
#define AAD_MAX_LEN             (12 + EAAD_MAX_LEN)
#define INFO_MAX_LEN            (2 * OSCORE_MAX_ID_LEN + 2 + 1 + 4 + 1 + 3)
...
void xor_arrays(const uint8_t *s1, const uint8_t *s2, uint8_t *dst, uint8_t len);
...
void oscore_init_security_context(oscore_security_context_t *ctx,
    ...
    if (senderIDLen > OSCORE_MAX_ID_LEN || recipientIDLen > OSCORE_MAX_ID_LEN) {
        return;
    }
                                ...
                                uint8_t length)
{
    ...
    if (type == OSCORE_DERIVATION_TYPE_KEY) {
        infoLen += cborencoder_put_text(&info[infoLen], (char *) key, sizeof(key) - 1);
    } else if (type == OSCORE_DERIVATION_TYPE_IV) {
